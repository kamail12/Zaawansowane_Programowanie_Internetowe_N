...
public class AccountApiController : BaseApiController
{
    ...
    public async Task<IActionResult> Login([FromForm] LoginUserVm applicationUser)
    {
        try
        {
            ...
            var result = await _signInManager.PasswordSignInAsync(applicationUser.Login, applicationUser.Password, true, false);//Próbuje zalogować określoną kombinację nazwy użytkownika i hasła jako operację asynchroniczną.
            if (result.Succeeded == false)
            {
                Logger.LogInformation($"Invalid username ({applicationUser.Login}) or password ({applicationUser.Password})");
                return BadRequest("Invalid credentials");
            }
            var user = await _userManager.FindByEmailAsync(applicationUser.Login);//Pobiera użytkownika, jeśli istnieje, skojarzonego z wartością określonego adresu e-mail.
            var userRoles = await _userManager.GetRolesAsync(user); //Pobiera listę nazw ról, do których należy określony użytkownik
